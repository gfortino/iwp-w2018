...
// INSTRUCTOR: Lab. 1 grade = 2.2
	...
	// INSTRUCTOR: The shopping cart may have multiple items or none.
	// INSTRUCTOR: The following line implies one and only one item.
	// INSTRUCTOR: Maybe put in a loop?  -0.1
	$total_ht=$shop_card['unit_price']*$shop_card['quantity'];

	// INSTRUCTOR: QST only applies to province of Quebec.  -0.1
	$tax=$shop_card['unit_price']*$shop_card['quantity']*(0.05+0.09975);
	
	// INSTRUCTOR: Specs indicate total before tax should be less than 35.  -0.1
	if($tax<35){
		$tax+=10;
	}
