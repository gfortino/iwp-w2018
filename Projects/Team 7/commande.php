...
<button onClick="window.open('http://localhost/dashboard/Myfolder/pay.php','main')">Pay</button><br><br>
<?php
...
$dbhost = 'localhost';
		 ...
		 $x2='0';//we initialize the variables for unit price
		 $y2='0';
		 $z2='0';
		 $w2='0';
		 $v2='0';
		 ...
		 $request='SELECT prix FROM produits WHERE id=1'; 
	$result=mysqli_query ($conn,$request);
	while ($row = $result->fetch_assoc()) { //we take the price from the game 1
    $x2=$row['prix'];
	}
	
	$request='SELECT prix FROM produits WHERE id=2'; 
	$result=mysqli_query ($conn,$request);
	while ($row = $result->fetch_assoc()) { //we take the price from the game 2
    $y2=$row['prix'];
	}
	
	$request='SELECT prix FROM produits WHERE id=3'; 
	$result=mysqli_query ($conn,$request);
	while ($row = $result->fetch_assoc()) { //we take the price from the game 3
    $z2=$row['prix'];
	}
	
	$request='SELECT prix FROM produits WHERE id=4'; 
	$result=mysqli_query ($conn,$request);
	while ($row = $result->fetch_assoc()) { //we take the price from the game 4
    $w2=$row['prix'];
	}
	
	$request='SELECT prix FROM produits WHERE id=5'; 
	$result=mysqli_query ($conn,$request);
	while ($row = $result->fetch_assoc()) { //we take the price from the game 5
    $v2=$row['prix'];
	}
		 
		 $request='SELECT prix FROM produits WHERE id=6'; 
	$result=mysqli_query ($conn,$request);
	while ($row = $result->fetch_assoc()) { //we take the price from the game 6
    $u2=$row['prix'];
	}
...
<table border=\"1\" bgcolor=\"\" width=\"\">
   ...
   <tr>
	<td width=\"\">Final fantasy 12</td>
   	<td width=\"\"><div align=\"center\">$x2 $</div></td>
